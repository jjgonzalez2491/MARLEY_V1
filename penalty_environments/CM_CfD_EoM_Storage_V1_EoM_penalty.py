...
class CM_EoM(MultiAgentEnv):
	def __init__(self, max_t, short_t, VoLL,
			  ...
			  opportunity_cost,
			  n_tech_RES, CfD_price_cap, CfD_target, penalty_type, demand_growth_type,
			  average_failures,
              scenario_failures,
		...
		self.CfD_price_agents_virtual = np.zeros([self.n_tech])
		...
		self.capacity_merchant_virtual = np.zeros([self.n_tech]) 
		self.inv_age_merchant_virtual = np.zeros([self.n_tech])
		self.inv_age_cm_virtual = np.zeros([self.n_tech])
		self.inv_age_CfD_virtual = np.zeros([self.n_tech])
		self.capacity_cm_virtual = np.zeros([self.n_tech])
		self.capacity_merchant_uc_virtual = np.zeros([self.n_tech])
		self.capacity_cm_uc_virtual = np.zeros([self.n_tech])
		...
		self.inv_cost_accum_virtual = np.zeros([self.n_tech])
		self.inv_cost_accum_merchant_virtual = np.zeros([self.n_tech])
		self.inv_cost_accum_cm_virtual = np.zeros([self.n_tech])
		self.inv_cost_accum_CfD_virtual = np.zeros([self.n_tech])

		# Battery investment variables 

		self.capacity_merchant_battery = np.zeros([self.agents_g]) 	
		self.capacity_merchant_battery_virtual = 0
		self.capacity_merchant_battery_uc = np.zeros([self.agents_g])  
		self.capacity_merchant_battery_uc_virtual = 0
		self.capacity_merchant_battery_virtual = 0
		...
		self.inv_age_merchant_battery_virtual = 0
		self.inv_cost_accum_merchant_battery_virtual = 0
		...
		self.reward_tech_merchant_virtual = np.zeros([self.n_tech])
		self.reward_tech_cm_virtual = np.zeros([self.n_tech])
		self.reward_tech_CfD_virtual = np.zeros([self.n_tech])
		...
		self.accumulated_profit_merchant_virtual = np.zeros([self.n_tech])
		self.accumulated_profit_cm_virtual = np.zeros([self.n_tech])
		self.accumulated_profit_CfD_virtual = np.zeros([self.n_tech])
		...
		self.reward_tech_merchant_battery_virtual = 0
		...
		self.accumulated_profit_merchant_battery_virtual = 0
		...
		self.under_construction_virtual = {}

		self.virtual_agent_name = "Virtual_agent"

		self.under_construction_virtual.update({self.virtual_agent_name:np.zeros([1,6])})
	...
	def reset(self, *, seed=None, options=None):
		...
		self.penalty_cm = np.zeros([self.agents_g, self.n_tech])

		self.penalty_CfD = np.zeros([self.agents_g, self.n_tech])

		self.penalty_merchant_battery = np.zeros([self.agents_g])

		self.penalty_cm_battery = np.zeros([self.agents_g])
		...
		if self.penalty_type == 0:
			...
			self.CfD_target = self.CfD_target_original * 0

			self.CO2_tax = self.CO2_tax_original * 0

			self.CO2_tax_tech = self.CO2_tax_tech_original * 0

		elif self.penalty_type == 1:

			self.CfD_target = self.CfD_target_original * 1
			...
			self.CO2_tax = self.CO2_tax_original * 1

			self.CO2_tax_tech = self.CO2_tax_tech_original * 1
		...
		self.capacity_merchant_virtual = np.zeros([self.n_tech])
		self.capacity_cm_virtual = np.zeros([self.n_tech])
		self.capacity_CfD_virtual = np.zeros([self.n_tech])
		self.capacity_merchant_uc = np.zeros([self.agents_g, self.n_tech])
		self.capacity_cm_uc = np.zeros([self.agents_g, self.n_tech])
		...
		self.capacity_merchant_uc_virtual = np.zeros([self.n_tech])
		self.capacity_cm_uc_virtual = np.zeros([self.n_tech])
		self.capacity_cm_battery_uc_virtual = 0
		self.capacity_CfD_uc_virtual = np.zeros([self.n_tech])
		...
		self.inv_cost_accum_merchant_virtual = np.zeros([self.n_tech])
		self.inv_cost_accum_cm_virtual = np.zeros([self.n_tech])
		self.inv_cost_accum_cm_battery_virtual = 0
		self.inv_cost_accum_CfD_virtual = np.zeros([self.n_tech])
		...
		self.CfD_income_virtual = np.zeros([self.n_tech])
		self.CfD_price_pond_virtual = np.zeros([self.n_tech])

		self.cm_capacity_difference = np.zeros([self.agents_g, self.n_tech])
		self.cm_capacity_difference_2 = np.zeros([self.agents_g, self.n_tech])
		...
		self.reward_tech_merchant_battery_virtual = 0
		self.reward_tech_cm_battery_virtual = 0
		
		self.accumulated_profit_merchant_battery = 0
		self.accumulated_profit_cm_battery = 0

		self.accumulated_profit_merchant_battery_virtual = 0
		self.accumulated_profit_cm_battery_virtual = 0
		...
		self.cm_income_battery_virtual = 0
		self.cm_inv_options_battery_virtual = 0
		self.inv_age_cm_battery_virtual = 0
		self.capacity_cm_battery_virtual = 0 
		...
		for a in self.possible_agents_g:
			...
			for j in range(self.n_tech):
				...
				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 2] = np.clip((ind_installed_capacity)/(ind_total_installed_capacity + delta) * 2 - 1, -1, 1)

				# Total

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 3] = np.clip((ind_installed_capacity)/(total_installed_capacity + delta) * 2 - 1, -1, 1)

				# ind - Existing

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 4] = np.clip(ind_installed_capacity_existing / (ind_installed_capacity + delta) * 2 - 1, -1, 1)

				# total - existing

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 5] = np.clip((total_installed_capacity_existing)/(total_installed_capacity + delta) * 2 - 1, -1, 1)

				# ind - Merchant

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 6] = np.clip(ind_installed_capacity_merchant / (ind_installed_capacity + delta) * 2 - 1, -1, 1)

				# total - Merchant

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 7] = np.clip((total_installed_capacity_merchant)/(total_installed_capacity + delta) * 2 - 1, -1, 1)

				# ind - CM

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 8] = np.clip(ind_installed_capacity_cm / (ind_installed_capacity + delta) * 2 - 1, -1, 1)

				# total - CM

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 9] = np.clip((total_installed_capacity_cm)/(total_installed_capacity + delta) * 2 - 1, -1, 1)

				# ind - CfD

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 10] = np.clip(ind_installed_capacity_CfD / (ind_installed_capacity + delta) * 2 - 1, -1, 1)

				# total - CfD

				obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific)  + self.n_observation_resources + (self.n_observation_specific_2 * j) + 11] = np.clip((total_installed_capacity_CfD)/(total_installed_capacity + delta) * 2 - 1, -1, 1)
			...
			obs_temp[i, self.n_observations_general + (self.n_tech * self.n_observation_specific) + self.n_observation_resources + (self.n_observation_specific_2 * self.n_tech) + self.n_observation_specific_2_battery + self.n_observation_time + 4] = np.clip(((np.sum(self.CfD_price_pond)/(np.sum(self.capacity_CfD) + delta))/self.CfD_price_cap) * 2 - 1, -1, 1)
	...
	def step(self, action_dict):
		...
		# Each step runs over a full week dispatch
			
		reward_temp = np.zeros(self.agents_g)

		# reward per technology
  
		profit_tech = np.zeros([self.agents_g, self.n_tech])

		obs_temp = np.zeros([self.agents_g, self.n_obs_g])

		q_accepted_u_cm = np.zeros([2])

		q_accepted_g_cm = np.zeros(int(self.agents_g * self.n_obs_g))
		...
		if self.cm_auction_indicator and self.month == 7:
			...
			## Virtual investments - Capacity Market
				
			a = self.virtual_agent_name

			inv_under_construction = self.under_construction_virtual.get(a)
			...
			for j in range(self.act_bids_cm):
				...
				if dummy_cm[j] < 0.02:
					...
					if j < self.n_tech: 
						...
						cc_inv_total = (self.inv_max[j]/(self.step_g_inv))
						...
						cc_inv_total = (self.inv_max_battery/(self.step_g_inv))

					# Vector for new investment (if action was taken to invest). All winning projects are built

					if self.cm_tech_cc[0,j] > 0.02 and self.max_year - self.year >  construction_time:
						...
						inv_vector[0,2] = construction_time
						inv_vector[0,3] = 1
						inv_vector[0,4] = np.max(self.cm_price)
						inv_vector[0,5] = cc_inv_total * inv_cost / self.normalization_factor
						...
						if j < self.n_tech: 
							...
							self.capacity_cm_uc_virtual[j] += cc_inv_total

							# Accumulated costs of investment in a particular technology

							self.inv_cost_accum_cm_virtual[j] += inv_vector[0,5]
							
						else:

							# Accumulated cm investment

							self.capacity_cm_battery_uc_virtual += cc_inv_total

							# Accumulated costs of investment in a particular technology

							self.inv_cost_accum_cm_battery_virtual += inv_vector[0,5]
							
						# Updating 
					
			self.under_construction_virtual.update({a:inv_under_construction})

		elif not self.cm_auction_indicator and self.month == 7:
		...
		if self.CfD_auction_indicator and self.month == 7:
			...
			# Virtual Investemntes - Contracts for Difference
			...
			for j in range(self.act_bids_CfD):
				...
				if dummy_CfD[j]  < 0.02:
					...
					if self.max_year - self.year >  self.construction_time[j]:

						# Vector for new investment (if action was taken to invest). All winning projects are built

						CfD_inv_total = (self.inv_max[j]/(self.step_g_inv))
						...
						inv_vector[0,2] = self.construction_time[j]
						inv_vector[0,3] = 2
						inv_vector[0,4] = np.max(self.CfD_price)
						inv_vector[0,5] = CfD_inv_total * self.inv_cost[self.year_int, j] / self.normalization_factor
						...
						self.capacity_CfD_uc_virtual[j] += CfD_inv_total

						# Accumulated costs of investment in a particular technology

						self.inv_cost_accum_virtual[j] += inv_vector[0,5]
						self.inv_cost_accum_CfD_virtual[j] += inv_vector[0,5]

						# Updating 
					
			self.under_construction_virtual.update({a:inv_under_construction})
			
		elif not self.CfD_auction_indicator and self.month == 7:
		...
		dummy_merchant = np.zeros([self.act_inv])
				...
				for j in range(self.act_inv):
					...
					if action_temp[i,j] > 0:
						...
						if m_inv_temp[i, j] > 0 and self.max_year - self.year >  construction_time:
							...
							dummy_merchant[j] += m_inv_temp[i, j] 
		...
		if self.month == 1:
			...
			for j in range(self.act_inv):
				...
				if dummy_merchant[j] < 0.02:
					...
					m_inv_temp = (inv_max/(self.step_g_inv))

					# Vector for new investment (if action was taken to invest)
								
					if self.max_year - self.year >  construction_time:
								
						inv_vector = np.zeros([1,6])
						inv_vector[0,0] = j
						inv_vector[0,1] = m_inv_temp
						inv_vector[0,2] = construction_time
						inv_vector[0,3] = 0
						inv_vector[0,4] = 0
						inv_vector[0,5] = m_inv_temp * inv_cost/ self.normalization_factor
						...
						if j  < self.n_tech:
							...
							self.capacity_merchant_uc_virtual[j] += m_inv_temp

							# Accumulated costs of investment in a particular technology

							self.inv_cost_accum_virtual[j] += inv_vector[0,5]
							self.inv_cost_accum_merchant_virtual[j] += inv_vector[0,5]
						
						else: 

							# Accumulated merchant investment

							self.capacity_merchant_battery_uc_virtual += m_inv_temp

							# Accumulated costs of investment in a particular technology

							self.inv_cost_accum_merchant_battery_virtual += inv_vector[0,5]
			...
			for vector_under_construction in inv_under_construction:
				...
				if inv_under_construction[k, 2] <= 0 and int(vector_under_construction[0]) < self.n_tech:
					...
					if vector_under_construction[3] == 0:
						...
						self.inv_age_merchant_battery[i] = self.inv_age_merchant_battery[i] * self.capacity_merchant_battery[i]/(vector_under_construction[1] + self.capacity_merchant_battery[i] + delta)
						...
						self.inv_age_cm_battery[i] = self.inv_age_cm_battery[i] * self.capacity_cm_battery[i]/(vector_under_construction[1] + self.capacity_cm_battery[i] + delta)
		...
		for vector_under_construction in inv_under_construction:
			...
			if inv_under_construction[k, 2] <= 0 and int(vector_under_construction[0]) < self.n_tech:
				...
				if vector_under_construction[3] == 1:
					...
					self.inv_age_cm_virtual[int(vector_under_construction[0])] = self.inv_age_cm_virtual[int(vector_under_construction[0])] * self.capacity_cm_virtual[int(vector_under_construction[0])]/(vector_under_construction[1] + self.capacity_cm_virtual[int(vector_under_construction[0])] + delta)

					self.cm_income_virtual[int(vector_under_construction[0])] += vector_under_construction[1] * vector_under_construction[4] * self.cm_tech_cc[0, int(vector_under_construction[0])] * self.short_t * self.hour_month / self.normalization_factor
						
					self.cm_inv_options_virtual[int(vector_under_construction[0])] += vector_under_construction[1] * self.cm_tech_cc[0, int(vector_under_construction[0])] 

					self.capacity_cm_virtual[int(vector_under_construction[0])] += vector_under_construction[1]

					self.inv_cost_accum_cm_virtual[int(vector_under_construction[0])] -= vector_under_construction[5]
						
				elif vector_under_construction[3] == 2:

					self.inv_age_CfD_virtual[int(vector_under_construction[0])] = self.inv_age_CfD_virtual[int(vector_under_construction[0])] * self.capacity_CfD_virtual[int(vector_under_construction[0])]/(vector_under_construction[1] + self.capacity_CfD_virtual[int(vector_under_construction[0])] + delta)

					self.capacity_CfD_virtual[int(vector_under_construction[0])] += vector_under_construction[1]

					self.CfD_price_pond_virtual[int(vector_under_construction[0])] += vector_under_construction[1] * vector_under_construction[4]

					self.CfD_price_agents_virtual[int(vector_under_construction[0])] = self.CfD_price_pond_virtual[int(vector_under_construction[0])]/self.capacity_CfD_virtual[int(vector_under_construction[0])] 

					self.inv_cost_accum_CfD_virtual[int(vector_under_construction[0])] -= vector_under_construction[5]
						
				elif vector_under_construction[3] == 0:

					self.inv_age_merchant_virtual[int(vector_under_construction[0])] = self.inv_age_merchant_virtual[int(vector_under_construction[0])] * self.capacity_merchant_virtual[int(vector_under_construction[0])]/(vector_under_construction[1] + self.capacity_merchant_virtual[int(vector_under_construction[0])] + delta)

					self.capacity_merchant_virtual[int(vector_under_construction[0])] += vector_under_construction[1]

					self.inv_cost_accum_merchant_virtual[int(vector_under_construction[0])] -= vector_under_construction[5]
						
				# Investment costs are deducted from vector

				self.inv_cost_accum_virtual[int(vector_under_construction[0])] -= vector_under_construction[5]

				# if np.sum(finished_indices) > 1:
				...
				if vector_under_construction[3] == 0:
					...
					self.inv_age_merchant_battery_virtual = self.inv_age_merchant_battery_virtual * self.capacity_merchant_battery_virtual/(vector_under_construction[1] + self.capacity_merchant_battery_virtual + delta)

					self.capacity_merchant_battery_virtual += vector_under_construction[1]

					self.inv_cost_accum_merchant_battery_virtual -= vector_under_construction[5]
					
				else:

					self.cm_income_battery_virtual += vector_under_construction[1] * vector_under_construction[4] * self.cm_tech_cc[0, int(vector_under_construction[0])] * self.short_t * self.hour_month / self.normalization_factor

					self.cm_inv_options_battery_virtual += vector_under_construction[1] * self.cm_tech_cc[0, int(vector_under_construction[0])] 

					self.inv_age_cm_battery_virtual = self.inv_age_cm_battery_virtual * self.capacity_cm_battery_virtual/(vector_under_construction[1] + self.capacity_cm_battery_virtual + delta)

					self.capacity_cm_battery_virtual += vector_under_construction[1]

					self.inv_cost_accum_cm_battery_virtual -= vector_under_construction[5]
		...
		for a in self.possible_agents_g:
			...
			adjusted_inflow[i] = (SoC_target[i] - self.SoC_merchant_storage_lt[i])/(self.capacity_existing_storage_lt[i] * self.short_t * self.hour_month + delta)
		...
		q_g_b = np.zeros(self.agents_g)
		p_g_b = np.zeros(self.agents_g)
		...
		for t in range (self.short_t):
			...
			price, q_accepted_u_flatt, q_accepted_g_flatt, LL = self.double_side_auction(p_u_t,q_u_t,p_g_t,q_g_t, self.random_g_p, self.VoLL)
			...
			for a in self.possible_agents_g:
				...
				for j in range(self.n_tech):
					...
					profit_tech[i,j] += profit_temp_spot
					...
					CfD_option_value = (price - self.CfD_price_agents[i,j]) * q_accepted_g[i,j] * self.capacity_CfD[i,j]/(self.capacity_merchant[i,j] + self.capacity_cm[i,j] + self.capacity_CfD[i,j] + self.inv_g_aging[i,j] + delta)/self.normalization_factor * self.hour_month

					spot_payment_trimester -= (price - self.CfD_price_agents[i,j]) * q_accepted_g[i,j] * self.capacity_CfD[i,j]/(self.capacity_merchant[i,j] + self.capacity_cm[i,j] + self.capacity_CfD[i,j] + self.inv_g_aging[i,j] + delta)
					spot_payment_hour -= (price - self.CfD_price_agents[i,j]) * q_accepted_g[i,j] * self.capacity_CfD[i,j]/(self.capacity_merchant[i,j] + self.capacity_cm[i,j] + self.capacity_CfD[i,j] + self.inv_g_aging[i,j] + delta)

					self.cm_capacity_difference[i,j] += -CfD_option_value + (profit_temp_spot * self.capacity_CfD[i,j]/(self.capacity_merchant[i,j] + self.capacity_cm[i,j] + self.capacity_CfD[i,j] + self.inv_g_aging[i,j] + delta))
					...
					self.total_production_tech[j] += q_accepted_g[i,j] * self.hour_month
				...
				if self.year > self.max_year - self.max_year/4:
					...
					self.accumulated_profit_merchant_battery += income_storage_merchant * (self.capacity_merchant_battery[i] + self.inv_init_battery[i]) /(self.capacity_merchant_battery[i] + self.capacity_cm_battery[i] + self.inv_init_battery[i] + delta)
					self.accumulated_profit_cm_battery += income_storage_cm * self.capacity_cm_battery[i] /(self.capacity_merchant_battery[i] + self.capacity_cm_battery[i] + self.inv_init_battery[i] + delta)
			...
			self.hourly_prices_net[t] = spot_payment_hour / (demand_hour + delta)

			# Profit from virtual agents

			for j in range(self.n_tech):

				if price >  (self.v_c_g[self.hour_year, j] + self.CO2_tax_tech[self.year_int, j]):

					q_accepted_g_virtual_merchant = (self.availability_tech_step[t, j]) * self.average_failures[0,j] * self.capacity_merchant_virtual[j]

					q_accepted_g_virtual_cm = (self.availability_tech_step[t, j]) * self.average_failures[0,j] * self.capacity_cm_virtual[j]

					q_accepted_g_virtual_CfD = (self.availability_tech_step[t, j]) * self.average_failures[0,j] * self.capacity_CfD_virtual[j]
				
				else: 

					q_accepted_g_virtual_merchant = 0

					q_accepted_g_virtual_cm = 0

					q_accepted_g_virtual_CfD = 0

				profit_cm_virtual = (((price - 
					  self.v_c_g[self.hour_year, j] - 
					  self.CO2_tax_tech[self.year_int, j]) * 
						q_accepted_g_virtual_cm) - self.fixed_cost[self.year_int, j] * self.capacity_cm_virtual[j])/(self.normalization_factor) * self.hour_month

				profit_CfD_virtual = (((price - 
					  self.v_c_g[self.hour_year, j] - 
					  self.CO2_tax_tech[self.year_int, j]) * 
						q_accepted_g_virtual_CfD) - self.fixed_cost[self.year_int, j] * self.capacity_CfD_virtual[j])/(self.normalization_factor) * self.hour_month

				profit_merchant_virtual = (((price - 
					  self.v_c_g[self.hour_year, j] - 
					  self.CO2_tax_tech[self.year_int, j]) * 
						q_accepted_g_virtual_merchant) - self.fixed_cost[self.year_int, j] * self.capacity_merchant_virtual[j])/(self.normalization_factor) * self.hour_month

				self.reward_tech_cm_virtual[j] += profit_cm_virtual * self.discount_factor_cm
				self.reward_tech_CfD_virtual[j] += profit_CfD_virtual * self.discount_factor_CfD 
				self.reward_tech_merchant_virtual[j] += profit_merchant_virtual * self.discount_factor_merchant 

				cm_option_value_virtual = np.maximum(price - self.cm_option_strike, 0) * self.cm_inv_options_virtual[j] /(self.normalization_factor) * self.hour_month
				CfD_option_value_virtual = (price - self.CfD_price_agents_virtual[j]) * q_accepted_g_virtual_CfD /(self.normalization_factor) * self.hour_month

				self.reward_tech_cm_virtual[j] -= cm_option_value_virtual * self.discount_factor_cm 
				self.reward_tech_CfD_virtual[j] -= CfD_option_value_virtual * self.discount_factor_CfD 

				if self.max_year - self.max_year/4:
					
					self.accumulated_profit_merchant_virtual[j] += profit_merchant_virtual
					
					self.accumulated_profit_cm_virtual[j] += profit_cm_virtual
					
					self.accumulated_profit_cm_virtual[j] -= cm_option_value_virtual
					
					self.accumulated_profit_CfD_virtual[j] += profit_CfD_virtual 
					
					self.accumulated_profit_CfD_virtual[j] -= CfD_option_value_virtual

			## Battery virtual profit

			q_accepted_u_merchant_virtual = self.strategy_charge_4_hours[t] * self.capacity_merchant_battery_virtual

			q_accepted_g_merchant_virtual = self.strategy_discharge_4_hours[t] * self.capacity_merchant_battery_virtual

			income_storage_merchant_virtual = (price * (
					-q_accepted_u_merchant_virtual + q_accepted_g_merchant_virtual) - self.fixed_cost_battery[self.year_int] * self.capacity_merchant_battery_virtual)/(self.normalization_factor) * self.hour_month 
				
			self.reward_tech_merchant_battery_virtual += income_storage_merchant_virtual * self.discount_factor_merchant 

			q_accepted_u_cm_virtual = self.strategy_charge_cm[t] * self.capacity_cm_battery_virtual

			q_accepted_g_cm_virtual = self.strategy_discharge_cm[t] * self.capacity_cm_battery_virtual

			income_storage_cm_virtual = (price * (
					-q_accepted_u_cm_virtual + q_accepted_g_cm_virtual) - self.fixed_cost_battery[self.year_int] * self.capacity_cm_battery_virtual)/(self.normalization_factor) * self.hour_month 
				
			self.reward_tech_cm_battery_virtual += income_storage_cm_virtual * self.discount_factor_cm

			cm_option_value_battery_virtual = np.maximum(price - self.cm_option_strike, 0) * self.cm_inv_options_battery_virtual /(self.normalization_factor) * self.hour_month

			self.reward_tech_cm_battery_virtual -= cm_option_value_battery_virtual * self.discount_factor_cm

			if self.max_year - self.max_year/4:

				self.accumulated_profit_merchant_battery_virtual += income_storage_merchant_virtual

				self.accumulated_profit_cm_battery_virtual += income_storage_cm_virtual

				self.accumulated_profit_cm_battery_virtual -= cm_option_value_battery_virtual
		...
		for a in self.possible_agents_g:
			...
			for j in range(self.n_tech):
				...
				if i == self.agents_g - 1:

					self.reward_tech_cm_virtual[j] -= self.inv_cost_accum_cm_virtual[j] * self.discount_factor_cm 
					self.reward_tech_CfD_virtual[j] -= self.inv_cost_accum_CfD_virtual[j] * self.discount_factor_CfD 
					self.reward_tech_merchant_virtual[j] -= self.inv_cost_accum_merchant_virtual[j] * self.discount_factor_merchant 
				...
				if i == self.agents_g - 1:
					...
					self.reward_tech_cm_virtual[j] += self.cm_income_virtual[j] * self.discount_factor_cm 
				...
				if self.year > self.max_year - self.max_year/4:
					...
					if i == self.agents_g - 1:
						...
						self.accumulated_profit_cm_virtual[j] += self.cm_income_virtual[j]
			...
			if i == self.agents_g - 1:
				...
				self.reward_tech_merchant_battery_virtual -= self.inv_cost_accum_merchant_battery_virtual * self.discount_factor_merchant 
				self.reward_tech_cm_battery_virtual -= self.inv_cost_accum_cm_battery_virtual * self.discount_factor_cm
			...
			for j in range(self.n_tech):
				...
				if self.time >= self.max_t: 
					...
					npv_annuaity_MW_merchant = (self.accumulated_profit_merchant[j] / (np.sum(self.capacity_merchant[:,j]) + delta)) * ((1 - (1 + self.opportunity_cost_merchant)**(-(np.max(self.life_time[j] - int(self.inv_age_merchant[i,j]),0))))/(self.opportunity_cost_merchant))

					npv_annuaity_MW_cm = (self.accumulated_profit_cm[j] / (np.sum(self.capacity_cm[:,j]) + delta)) * ((1 - (1 + self.opportunity_cost_cm)**(-(np.max(self.life_time[j] - int(self.inv_age_cm[i,j]),0))))/(self.opportunity_cost_cm))

					npv_annuaity_MW_CfD = (self.accumulated_profit_CfD[j] / (np.sum(self.capacity_CfD[:,j]) + delta)) * ((1 - (1 + self.opportunity_cost_CfD)**(-(np.max(self.life_time[j] - int(self.inv_age_CfD[i,j]),0))))/(self.opportunity_cost_CfD))
					...
					if i == self.agents_g - 1:
						...
						npv_annuaity_MW_merchant = ((self.accumulated_profit_merchant[j] + self.accumulated_profit_merchant_virtual[j]) / (np.sum(self.capacity_merchant[:,j]) + self.capacity_merchant_virtual[j] + delta)) * ((1 - (1 + self.opportunity_cost_merchant)**(-(np.max(self.life_time[j] - int(self.inv_age_merchant_virtual[j]),0))))/(self.opportunity_cost_merchant))

						npv_annuaity_MW_cm = ((self.accumulated_profit_cm[j] + self.accumulated_profit_cm_virtual[j]) / (np.sum(self.capacity_cm[:,j]) + self.capacity_cm_virtual[j] + delta)) * ((1 - (1 + self.opportunity_cost_cm)**(-(np.max(self.life_time[j] - int(self.inv_age_cm_virtual[j]),0))))/(self.opportunity_cost_cm))

						npv_annuaity_MW_CfD = ((self.accumulated_profit_CfD[j] + self.accumulated_profit_CfD[j])/ (np.sum(self.capacity_CfD[:,j]) + self.capacity_CfD_virtual[j] + delta)) * ((1 - (1 + self.opportunity_cost_CfD)**(-(np.max(self.life_time[j] - int(self.inv_age_CfD_virtual[j]),0))))/(self.opportunity_cost_CfD))
						
						
						depreciation_value_merchant = npv_annuaity_MW_merchant * (self.capacity_merchant_virtual[j])/(self.count_last[j]) 

						depreciation_value_cm = npv_annuaity_MW_cm * (self.capacity_cm_virtual[j])/(self.count_last[j]) 

						depreciation_value_CfD = npv_annuaity_MW_CfD * (self.capacity_CfD_virtual[j])/(self.count_last[j])


						self.reward_tech_merchant_virtual[j] += depreciation_value_merchant * self.discount_factor_merchant

						self.reward_tech_cm_virtual[j] += depreciation_value_cm * self.discount_factor_cm

						self.reward_tech_CfD_virtual[j] += depreciation_value_CfD * self.discount_factor_CfD
						
			## Depreciation value battery

			if self.time >= self.max_t: 
					
				npv_annuaity_MW_merchant_battery = (self.accumulated_profit_merchant_battery / (np.sum(self.capacity_merchant_battery) + delta)) * ((1 - (1 + self.opportunity_cost_merchant)**(-(np.max(self.life_time_battery - int(self.inv_age_merchant_battery[i]),0))))/(self.opportunity_cost_merchant))
				...
				npv_annuaity_MW_cm_battery = (self.accumulated_profit_cm_battery / (np.sum(self.capacity_cm_battery) + delta)) * ((1 - (1 + self.opportunity_cost_cm)**(-(np.max(self.life_time_battery - int(self.inv_age_cm_battery[i]),0))))/(self.opportunity_cost_cm))
				...
				if i == self.agents_g - 1:
					...
					npv_annuaity_MW_merchant_battery = ((self.accumulated_profit_merchant_battery + self.accumulated_profit_merchant_battery_virtual)/ (np.sum(self.capacity_merchant_battery) + self.capacity_merchant_battery_virtual + delta)) * ((1 - (1 + self.opportunity_cost_merchant)**(-(np.max(self.life_time_battery - int(self.inv_age_merchant_battery_virtual),0))))/(self.opportunity_cost_merchant))

					depreciation_value_merchant_battery = npv_annuaity_MW_merchant_battery * (self.capacity_merchant_battery_virtual)/(self.count_last_battery) 

					self.reward_tech_merchant_battery_virtual += depreciation_value_merchant_battery * self.discount_factor_merchant

					# Capacity Market

					npv_annuaity_MW_cm_battery = ((self.accumulated_profit_cm_battery + self.accumulated_profit_cm_battery_virtual) / (np.sum(self.capacity_cm_battery) + self.capacity_cm_battery_virtual + delta)) * ((1 - (1 + self.opportunity_cost_cm)**(-(np.max(self.life_time_battery - int(self.inv_age_cm_battery_virtual),0))))/(self.opportunity_cost_cm))

					depreciation_value_cm_battery = npv_annuaity_MW_cm_battery * (self.capacity_cm_battery_virtual)/(self.count_last_battery) 

					self.reward_tech_cm_battery_virtual += depreciation_value_cm_battery * self.discount_factor_cm
			...
			self.normalized_merchant_reward = np.zeros([self.n_tech])

			self.normalized_cm_reward = np.zeros([self.n_tech])

			self.normalized_CfD_reward = np.zeros([self.n_tech])

			self.normalized_merchant_battery_reward = 0

			self.normalized_cm_battery_reward = 0

			self.dif_normalized_merchant_reward = np.zeros([self.n_tech])

			self.dif_normalized_cm_reward = np.zeros([self.n_tech])

			self.dif_normalized_CfD_reward = np.zeros([self.n_tech])

			self.dif_normalized_merchant_battery_reward = 0

			self.dif_normalized_cm_battery_reward = 0
			...
			if self.time >= self.max_t: 
				...
				for j in range(self.n_tech):
					...
					self.normalized_merchant_reward[j] = np.sum(self.reward_tech_merchant[:,j])/(np.sum(self.capacity_merchant_uc[:,j]) + delta)

					self.normalized_cm_reward[j] = np.sum(self.reward_tech_cm[:,j])/(np.sum(self.capacity_cm_uc[:,j]) + delta)

					self.normalized_CfD_reward[j] = np.sum(self.reward_tech_CfD[:,j])/(np.sum(self.capacity_CfD_uc[:,j]) + delta)

				self.normalized_merchant_battery_reward = np.sum(self.reward_tech_merchant_battery)/(np.sum(self.capacity_merchant_battery_uc) + delta)

				self.normalized_cm_battery_reward = np.sum(self.reward_tech_cm_battery)/(np.sum(self.capacity_cm_battery_uc) + delta) 
			
			i += 1

		self.average_rewards_merchant = np.zeros([self.n_tech])

		self.average_rewards_cm = np.zeros([self.n_tech])

		self.average_rewards_CfD = np.zeros([self.n_tech])

		if self.time >= self.max_t: 

			for j in range(self.n_tech):
				
				# Mask for enabled investments
				investments_enabled_mask = self.investments_enabled[:, j] == 1

				# Calculate averages for each reward type using masked values
				self.average_rewards_merchant[j] = np.maximum(np.mean(self.reward_tech_merchant[investments_enabled_mask, j]), self.reward_tech_merchant_virtual[j])
				self.average_rewards_cm[j] = np.maximum(np.mean(self.reward_tech_cm[investments_enabled_mask, j]), self.reward_tech_cm_virtual[j])
				self.average_rewards_CfD[j] = np.maximum(np.mean(self.reward_tech_CfD[investments_enabled_mask, j]), self.reward_tech_CfD_virtual[j])

			investments_enabled_battery = self.investments_enabled[:,self.n_tech] 

			merchant_masked_battery = self.reward_tech_merchant_battery[investments_enabled_battery == 1]

			self.average_rewards_merchant_battery = np.maximum(np.mean(merchant_masked_battery), self.reward_tech_merchant_battery_virtual) 

			cm_masked_battery = self.reward_tech_cm_battery[investments_enabled_battery == 1]

			self.average_rewards_cm_battery =  np.maximum(np.mean(cm_masked_battery), self.reward_tech_cm_battery_virtual)  

		## Penalty and reward update
			...
			if self.time >= self.max_t: 
				...
				for j in range(self.n_tech):
					...
					if self.average_rewards_merchant[j] > 0 and self.investments_enabled[i,j] == 1 and self.average_rewards_merchant[j] > self.reward_tech_merchant[i,j]:
						...
						self.penalty_merchant[i,j] = self.average_rewards_merchant[j] - self.reward_tech_merchant[i,j]
						
						self.reward_tech_merchant_step[i,j] -= self.penalty_merchant[i,j]

					if self.average_rewards_cm[j] > 0 and self.investments_enabled[i,j] == 1 and self.average_rewards_cm[j] > self.reward_tech_cm[i,j]:

						self.penalty_cm[i,j] = self.average_rewards_cm[j] - self.reward_tech_cm[i,j]
						
						self.reward_tech_cm_step[i,j] -= self.penalty_cm[i,j]

					if self.average_rewards_CfD[j] > 0 and self.investments_enabled[i,j] == 1 and self.average_rewards_CfD[j] > self.reward_tech_CfD[i,j]:

						self.penalty_CfD[i,j] = self.average_rewards_CfD[j] - self.reward_tech_CfD[i,j]
						
						self.reward_tech_CfD_step[i,j] -= self.penalty_CfD[i,j]

				if self.average_rewards_merchant_battery > 0 and self.investments_enabled[i,self.n_tech] == 1 and self.average_rewards_merchant_battery > self.reward_tech_merchant_battery[i]:

					self.penalty_merchant_battery[i] = self.average_rewards_merchant_battery - self.reward_tech_merchant_battery[i]
						
					self.reward_tech_merchant_battery_step[i] -= self.penalty_merchant_battery[i]

				if self.average_rewards_cm_battery > 0 and self.investments_enabled[i,self.n_tech] == 1 and self.average_rewards_cm_battery > self.reward_tech_cm_battery[i]:

					self.penalty_cm_battery[i] = self.average_rewards_cm_battery - self.reward_tech_cm_battery[i]
						
					self.reward_tech_cm_battery_step[i] -= self.penalty_cm_battery[i]

			
			self.penalty_term[i,:self.n_tech] += self.penalty_merchant[i,:] + self.penalty_cm[i,:] + self.penalty_CfD[i,:]

			self.penalty_term[i,self.n_tech] += self.penalty_merchant_battery[i] + self.penalty_cm_battery[i]
		...
		if self.month >= self.yearly_resolution:
			...
			self.inv_age_cm_virtual[self.capacity_cm_virtual > 0] += 1

			self.inv_age_CfD_virtual[self.capacity_CfD_virtual > 0] += 1

			self.inv_age_merchant_virtual[self.capacity_merchant_virtual > 0] += 1

			self.inv_age_merchant_battery[self.capacity_merchant_battery > 0] += 1

			self.inv_age_cm_battery[self.capacity_cm_battery > 0] += 1

			if self.capacity_merchant_battery_virtual > 0:

				self.inv_age_merchant_battery_virtual += 1

			if self.capacity_cm_battery_virtual  > 0:

				self.inv_age_cm_battery_virtual += 1
	...
	def get_action_masks_g(self, investments_enabled, capacity_existing_storage_lt):
		...
		for j in range(self.act_bids_cm):
			...
			if self.month != 7 or self.cm_auction_indicator == False or self.cm_tech_cc[0,j] <= 0.02 or self.max_year - self.year - 5 <= construction_time or investments_enabled[j] == 0:
		...
		for j in range(self.act_bids_CfD):
			...
			if self.month != 7 or self.CfD_auction_indicator == False or self.max_year - self.year - 5 <= self.construction_time[j] or investments_enabled[j] == 0:
	...
	## Capacity market balance function
	...
	## Function for definiyng the storage priorities
	# Priority to shorter-term storage
